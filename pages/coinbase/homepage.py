...
class HomePage:
    ...
    bitcoin_buy_button_loc=            (
        by.XPATH, "//a[@href='/price/bitcoin']/button")

    ethereum_text_link_loc =           (by.XPATH, "//h4[.='Ethereum']")
    ethereum_ETH_link_loc =            (by.XPATH, "//h4[.='ETH']") 
    ethereum_logo_link_loc =           (
        by.XPATH, "//img[@alt='Ethereum logo']")
    ethereum_buy_button_loc =          (
        by.XPATH, "//a[@href='/price/ethereum']/button")

    bitcoinCash_text_link_loc =        (by.XPATH, "//h4[.='Bitcoin Cash']")        
    bitcoinCash_BCH_link_loc =         (by.XPATH, "//h4[.='BCH']")
    bitcoinCash_logo_link_loc =        (
        by.XPATH, "//img[@alt='Bitcoin Cash logo']")
    bitcoinCash_buy_button_loc =       (
        by.XPATH, "//a[@href='/price/bitcoin-cash']/button")

    litecoin_text_link_loc =           (
        by.XPATH, "//h4[.='Litecoin']")
    litecoin_LTC_link_loc =            (by.XPATH, "//h4[.='LTC']")
    litecoin_logo_link_loc =           (
        by.XPATH, "//img[@alt='Litecoin logo']")
    litecoin_buy_button_loc =          (
        by.XPATH, "//a[@href='/price/litecoin']/button")
    ...
    android_app_link_loc =             (
        by.XPATH, "//a[contains(@href,'android')]")
    ...
    language_menu_loc =                (
        by.XPATH,"//select[contains(@class,'Language')]")
    ...
    what_is_blockchain_link_loc =      (
        by.XPATH, "//a[.='What is Blockchain?']")
    ...
    def build_elements(self):
        ...
        self.home_link =                            Link(
            self.driver, self.home_link_loc)
        self.prices_link =                          Link(
            self.driver, self.prices_link_loc)
        self.products_menu =                        ReactiveMenu(
            self.driver,self.products_menu_loc)
        self.company_menu =                         ReactiveMenu(
            self.driver, self.company_menu_loc)
        self.earn_crypto_link=                      Link(
            self.driver, self.earn_crypto_link_loc)
        self.sign_in_link =                         Link(
            self.driver, self.sign_in_link_loc)
        ...
        self.bitcoin_text_link =                    Link(
            self.driver,self.bitcoin_text_link_loc)
        self.bitcoin_BTC_link =                     Link(
            self.driver,self.bitcoin_BTC_link_loc)
        self.bitcoin_logo_link =                    Link(
            self.driver,self.bitcoin_logo_link_loc)
        self.bitcoin_buy_button =                   Button(
            self.driver, self.bitcoin_buy_button_loc)

        self.ethereum_text_link =                   Link(
            self.driver,self.ethereum_text_link_loc)
        self.ethereum_ETH_link =                    Link(
            self.driver,self.ethereum_ETH_link_loc)
        self.ethereum_logo_link =                   Link(
            self.driver,self.ethereum_logo_link_loc)
        ...
        self.litecoin_text_link =                   Link(
            self.driver,self.litecoin_text_link_loc)
        self.litecoin_LTC_link =                    Link(
            self.driver,self.litecoin_LTC_link_loc)
        self.litecoin_logo_link =                   Link(
            self.driver,self.litecoin_logo_link_loc)
        ...
        self.maker_link =                           Link(
            self.driver,self.maker_link_loc)
        self.celo_link =                            Link(
            self.driver,self.celo_link_loc)
        self.compound_link =                        Link(
            self.driver,self.compound_link_loc)
        self.view_more_link =                       Link(
            self.driver,self.view_more_link_loc)

        # MOBILE APPS

        self.android_app_link =                     Link(
            self.driver,self.android_app_link_loc)
        self.ios_app_link =                         Link(
            self.driver,self.ios_app_link_loc)
        ...
        self.language_menu =                        ReactiveMenu(
            self.driver,self.language_menu_loc)

        # PRODUCTS FOOTER

        self.coinbase_link =                        Link(
            self.driver,self.coinbase_link_loc)
        self.commerce_link =                        Link(
            self.driver,self.commerce_link_loc)
        self.custody_link =                         Link(
            self.driver,self.custody_link_loc)
        self.earn_link =                            Link(
            self.driver,self.earn_link_loc)
        self.pro_link =                             Link(
            self.driver,self.pro_link_loc)
        self.usd_coin_link =                        Link(
            self.driver,self.usd_coin_link_loc)
        self.wallet_link =                          Link(
            self.driver,self.wallet_link_loc)
        self.ventures_link =                        Link(
            self.driver,self.ventures_link_loc)

        # LEARN FOOTER

        self.browse_assets_link =                   Link(
            self.driver,self.browse_assets_link_loc)
        ...
        self.buy_bitcoin_link =                     Link(
            self.driver,self.buy_bitcoin_link_loc)
        self.buy_bitcoinCash_link =                 Link(
            self.driver, self.buy_bitcoinCash_link_loc)
        self.buy_ethereum_link =                    Link(
            self.driver,self.buy_ethereum_link_loc)
        self.buy_litecoin_link =                    Link(
            self.driver,self.buy_litecoin_link_loc)
        self.buy_XRP_link =                         Link(
            self.driver,self.buy_XRP_link_loc)
        self.supported_countries_link =             Link(
            self.driver, self.supported_countries_link_loc)
        self.status_link =                          Link(
            self.driver,self.status_link_loc)   
        self.taxes_link =                           Link(
            self.driver,self.taxes_link_loc)

        # COMPANY FOOTER

        self.about_link =                           Link(
            self.driver,self.about_link_loc)
        self.affiliates_link =                      Link(
            self.driver,self.affiliates_link_loc)
        self.careers_link =                         Link(
            self.driver,self.careers_link_loc)
        self.partners_link =                        Link(
            self.driver,self.partners_link_loc)
        self.press_link =                           Link(
            self.driver,self.press_link_loc)
        self.legal_and_privacy_link =               Link(
            self.driver, self.legal_and_privacy_link_loc)
        self.cookie_policy_link =                   Link(
            self.driver,self.cookie_policy_link_loc)
        self.support_link =                         Link(
            self.driver,self.support_link_loc)

        # SOCIAL FOOTER

        self.blog_link =                            Link(
            self.driver,self.blog_link_loc)
        self.twitter_link =                         Link(
            self.driver,self.twitter_link_loc)
        self.facebook_link =                        Link(
            self.driver,self.facebook_link_loc)
